...
export async function getTwilioToken(): Promise<TwilioToken> {
  const tokenUrl = (import.meta.env as any).TOKEN_WEBHOOK_URL || 'https://omniassistai.com/token';
  try { 
...
export async function sendSMS(to: string, message: string): Promise<SMSResponse> {
    ...
    const smsUrl = (import.meta.env as any).SMS_WEBHOOK_URL || 'https://omniassistai.com/sms';
    try {
